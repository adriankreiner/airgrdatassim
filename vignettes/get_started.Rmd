...
After the study of @Piazzi_2020, airGRdatassim package provides functions allowing to perform DA-based ensemble discharge simulations. airGRdatassim is a package based on the [airGR](https://CRAN.R-project.org/package=airGR) hydrological modeling package (@R-airGR, @airGR2017). This package is designed to flexibly use the Ensemble Kalman filter (EnKF) or Particle filter (PF) technique to assimilate daily discharge observations into GR4J, GR5J, and GR6J models (@perrin_improvement_2003, @le_moine_bassin_2008, @pushpalatha_downward_2011). 
...
airGRdatassim is a package based on the [airGR](https://CRAN.R-project.org/package=airGR) hydrological modeling package (@R-airGR, @airGR2017).
...
Among the model states, the update of the routing store level ensures the most benefit from the assimilation of observed discharges. Conversely, the update of the unit hydrograph only has a slight impact on the accuracy of the DA-based discharge simulations (@Piazzi_2020).
...
When considering the uncertainty in the forcings, the `CreateInputsPerturb()` function generates an ensemble for precipitation or/and potential evapotranspiration, provided as function argument/s.
...
The example below considers EnKF-based discharge simulations relying on a 100-member ensemble, when accounting for the uncertainty in model state variables (see `IsState`). More specifically, only the levels of the production (`"Prod"`) and the routing (`"Rout"`) stores are updated and perturbed (see `IndState`).
...
When accounting for the uncertainty in meteorological forcings, the `CreateInputsPerturb()` function generates an ensemble for precipitation or/and potential evapotranspiration, provided as function argument/s.
