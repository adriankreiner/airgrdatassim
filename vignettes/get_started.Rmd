...
The perturbation of model states is performed through a normally-distributed null-mean noise. The noise variance is assumed equal to the variance of the state variables resulting from the analysis procedure and it is restricted between upper and lower limits (@Salamon_2009).
...
StateEnKF <- c("Prod", "Rout") # "Prod": production store level; Rout": routing store level; "UH": unit hydrograph
...
result_EnKF <- RunModel_DA(DaMethod = "EnKF", IndRun = IndRun, StateEnKF = StateEnKF,
                           NbMbr = NbMbr, FUN_MOD = RunModel_GR5J,
                           InputsModel = InputsModel, InputsPert = InputsPert, 
                           Param = Param, StatePert = StatePert, Qobs = BasinObs$Qmm)
...
```{r, warning=FALSE}
result_PF <- RunModel_DA(DaMethod = "PF", IndRun = IndRun,
                         NbMbr = NbMbr, FUN_MOD = RunModel_GR5J,
                         InputsModel = InputsModel, InputsPert = InputsPert, 
                         Param = Param, StatePert = "Rout", Qobs = BasinObs$Qmm)
```
