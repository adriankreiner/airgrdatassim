...
DA_PF <- function(Obs, Qsim, States, IsState, IndState, Param, VarThr, NbMbr) {
  ...
  if (IsState) {
    ...
    EnsState["UH"  , ] <- sapply(seq_along(States), function(x) States[[x]]$UH$UH2[1])
  ...
  for (iPart in seq_len(nrow(Repeats))) {
    ...
    if (IsState) { # state perturbation
      ...
      if (RepParticle > 1) {      # if the selected particle is replicated --> its replications are perturbed
        ...
        StateRep["UH", ]   <- rep(States[[IndexParticle]]$UH$UH2[1],  RepParticle)
        ...
        for (iRep in seq_len(RepParticle)) {
          ...
          TempStatePert[[iRep]]$UH$UH2[1]  <- StateRepPert["UH",   iRep]
        }
      } # END IF RepParticle 
